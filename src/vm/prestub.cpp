                                    ...
                                    DWORD *                     pDictionaryIndexAndSlot)
{
    ...
    if (kind == ENCODE_DICTIONARY_LOOKUP_METHOD)
    {
        ...
        pResult->helper = CORINFO_HELP_RUNTIMEHANDLE_METHOD_WITHSLOT;
    }
    else
    {
        ...
        pResult->helper = CORINFO_HELP_RUNTIMEHANDLE_CLASS_WITHSLOT;
    }
    ...
    *pDictionaryIndexAndSlot <<= 16;
    ...
    if (kind == ENCODE_DICTIONARY_LOOKUP_METHOD)
    {
        if (DictionaryLayout::FindToken(pModule->GetLoaderAllocator(), numGenericArgs, pContextMD->GetDictionaryLayout(), pResult, (BYTE*)pBlobStart, 1, &dictionarySlot))
        {
    ...
    {
        if (DictionaryLayout::FindToken(pModule->GetLoaderAllocator(), numGenericArgs, pContextMT->GetClass()->GetDictionaryLayout(), pResult, (BYTE*)pBlobStart, 2, &dictionarySlot))
        {
            pResult->testForNull = 1;
...
PCODE DynamicHelperFixup(TransitionBlock * pTransitionBlock, TADDR * pCell, DWORD sectionIndex, Module * pModule, CORCOMPILE_FIXUP_BLOB_KIND * pKind, TypeHandle * pTH, MethodDesc ** ppMD, FieldDesc ** ppFD)
{
    ...
    else
    {
        ...
        case ENCODE_DICTIONARY_LOOKUP_METHOD:
            {
                pHelper = DynamicHelpers::CreateDictionaryLookupHelper(pModule->GetLoaderAllocator(), &genericLookup, dictionaryIndexAndSlot);
            }
            break;
