            ...
            for (int rid = 0; rid < ridToStub.Count; rid++)
            {
                ...
                if (methodIL != null)
                {
                    BlobVertex localsSignatureBlob;
                    {
                        ...
                        localsSignatureBuilder.EmitByte((byte)CorElementType.ELEMENT_TYPE_END);
                    ...
                    ArrayBuilder<BlobVertex> tokenMap = new ArrayBuilder<BlobVertex>();
                    {
                        foreach(object objectForToken in methodIL.GetTokenObjects())
                        {
                            ...
                            {
                                sigBuilder.EmitByte((byte)ReadyToRunFixupKind.READYTORUN_FIXUP_PInvokeTarget);
                            }
                            else if (objectForToken is EcmaType type)
                            {
                                sigBuilder.EmitByte((byte)ReadyToRunFixupKind.READYTORUN_FIXUP_TypeHandle);
                            ...
                            else if (objectForToken is EcmaMethod method)
                            {
                                ...
                                sigBuilder.EmitByte((byte)ReadyToRunFixupKind.READYTORUN_FIXUP_MethodHandle);
